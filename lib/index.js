...
PaymentProtocol.verifyCertChain = function(chain, sigHashAlg) {
  ...
  return chain.every(function(cert, i) {
    ...
    var hashAlgString;
    var hashAlg = PaymentProtocol.getAlgorithm(c.signatureAlgorithm.algorithm, true);
    if (typeof (hashAlg) === 'object' && typeof (hashAlg.hash) !== 'undefined') {
        hashAlgString = hashAlg.cipher + '-' + hashAlg.hash;
    }
    else if (typeof (hashAlg) === 'object' && typeof (hashAlg.cipher) !== 'undefined') {
        hashAlgString = hashAlg.cipher + '-' + sigHashAlg;
    }
    else {
        hashAlgString = 'RSA-' + sigHashAlg;
    }
    ...
    var verifier = crypto.createVerify(hashAlgString);
    verifier.update(tbs);
